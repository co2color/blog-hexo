...
不过看到这个例子，vuer可能会怒吼：“mixin不是vue中的糟粕吗！你怎么还在用mixin！”

我曰：vue2中的mixin的坏处是把多个mixin都绑定到了一个this上面，导致来源不清晰、冲突等问题，然而跟该例子中的mixin是两件事，不要搞混了....该例子中每个mixin的内容自己控制，**用mixin的对象都是各自独立的**，要清楚这一点。

总结：继承想要好，就得一开始设计得好，父类最开始的时候就不能加太多的功能；而组合想要好，就挨个添加需要的功能，虽然一时半会代码量增多了，但是后面可以通过复用(比如该demo中charge和refuelf方法)来降低代码量。所以我认为组合的上限和下限都比继承高。
